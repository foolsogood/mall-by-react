class tool {
  public formatTime(time, type = 'ymdhms'): string {
    type timeType = string | number;
    const _formatFn = (unit: number) => (unit < 10 ? '0' + unit : unit);
    const t = new Date(time);
    const y = t.getFullYear();
    let month: timeType = t.getMonth() + 1;
    month = _formatFn(month);
    let d: timeType = t.getDay();
    d = _formatFn(d);
    let h: timeType = t.getHours();
    h = _formatFn(h);
    let min: timeType = t.getMinutes();
    min = _formatFn(min);
    let s: timeType = t.getSeconds();
    s = _formatFn(s);
    const op = {
  ...
  //比较两个数据是否相等
  public checkIfEual(objA, objB): boolean {
